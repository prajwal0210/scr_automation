...
RAW_DIR = BASE_DIR / "support_session_raw"
...
def run_support_session_pipeline(
    employer_contact_file,
    support_session_file,
    unit_result_file,
    ...
    """
    Support Session pipeline.

    Input files:
    1. Employer contact (primary).csv
    2. support_session_trainer_events_report.xlsx
    3. Unit Result Export Report.csv
    ...
    # =========================================================
    unit_result_output_file = output_folder / "support_session_unit_result_with_employee_contacts.xlsx"
    cleaned_events_output_file = output_folder / "support_session_cleaned_event_titles.xlsx"
    matched_output_file = output_folder / "support_session_events_matched_with_units.xlsx"
    ...
    def clean_support_event_title(text):
        """
        Support-session-specific event title cleaning.
        Removes generic phrases while preserving the student name.
        """
        if pd.isna(text):
            return ""
        ...
        patterns_to_remove = [
            r"(?i)\bsupport\s*session\b",
            r"(?i)\bsite\s*[-/]?\s*visit\b",
        ]
    ...
    def is_valid_student_name(text):
        """
        Keep likely student-name rows only.
        Reject generic support-session titles and obvious junk rows.
        """
        if pd.isna(text):
            return False
        ...
        # Remove separators for validation only
    ...
    # =========================================================
    # STEP 2: PROCESS SUPPORT SESSION EVENTS
    # =========================================================
    print("Step 2 started: Cleaning support session event titles...")

    events_df = pd.read_excel(support_session_file)
    events_df = clean_columns(events_df)

    required_event_cols = ["Date", "Time", "End", "Event Title", "Trainer Name", "Location"]
    missing_event_cols = [col for col in required_event_cols if col not in events_df.columns]
    if missing_event_cols:
        raise ValueError(f"Missing required columns in support session file: {missing_event_cols}")

    events_df["Event Title"] = events_df["Event Title"].apply(normalize_text)
    events_df["Cleaned Event Title"] = events_df["Event Title"].apply(clean_support_event_title)
    events_df["Is Valid Student Row"] = events_df["Cleaned Event Title"].apply(is_valid_student_name)

    # Keep student rows only
    events_df = events_df[events_df["Is Valid Student Row"]].copy()

    # Prefer rows that still contain 'support session' wording in raw title if duplicates exist
    def has_support_session(text):
        if pd.isna(text):
            return False
        text = str(text).lower()
        return "support session" in text

    events_df["has_support_session"] = events_df["Event Title"].apply(has_support_session)
    events_df = events_df.sort_values(by="has_support_session", ascending=False).copy()

    # Support session uniqueness should not be based only on student name.
    # Keep separate rows when date/time/trainer/location differ.
    events_df["Date_norm"] = pd.to_datetime(
        events_df["Date"], dayfirst=True, errors="coerce"
    ).dt.strftime("%d/%m/%Y")

    events_df["Time_norm"] = events_df["Time"].astype(str).str.strip()
    events_df["End_norm"] = events_df["End"].astype(str).str.strip()
    events_df["Trainer_norm"] = events_df["Trainer Name"].astype(str).str.strip().str.lower()
    events_df["Location_norm"] = events_df["Location"].astype(str).str.strip().str.lower()
    ...
    # STEP 3: FINAL MATCH
    # =========================================================
    print("Step 3 started: Matching support session events with units result...")
    ...
    if "Cleaned Event Title" not in left_match_df.columns:
        raise ValueError("Column 'Cleaned Event Title' not found in cleaned support session file.")
    ...
    # Remove fully blank student rows
    final_df = final_df[
        ~(
    ...
    # Keep only template columns + useful debug columns if needed
    final_df = final_df[
        required_template_cols
    ].copy()
    ...
    # FORMAT DATE COLUMN
    # -------------------------
    if "Date of Contact" in template_headers:
        col_idx = template_headers["Date of Contact"]

        for row_num in range(start_row, ws.max_row + 1):
            cell = ws.cell(row=row_num, column=col_idx)
            parsed_date = parse_date_value(cell.value)
...
if __name__ == "__main__":
    run_support_session_pipeline(
        ...
        support_session_file=RAW_DIR / "support_session_trainer_events_report.xlsx",
        unit_result_file=RAW_DIR / "Unit Result Export Report.csv",
    )